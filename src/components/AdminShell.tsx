...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-bg grid grid-cols-[240px_1fr]">
      <aside className="bg-bg-soft border-r border-line p-4 flex flex-col gap-1.5 hidden lg:flex">
      ...
      <div className="flex flex-col min-w-0">
        <header className="h-16 bg-bg-elev border-b border-line flex items-center px-8 gap-4">
          <div className="lg:hidden font-display text-lg text-ink">{COMPANY.name}</div>
          <div className="flex items-center gap-2 ml-auto">
            <span className="text-sm text-ink-3">Admin</span>
            <div className="w-8 h-8 rounded-full bg-pill-bg text-bg-soft flex items-center justify-center text-sm font-semibold">A</div>
          </div>
        </header>
